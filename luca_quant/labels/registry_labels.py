...
def register_label(name: str, task: str = "classification"):
    """
    task: "classification" | "regression"

    Khai báo task ngay ở registry là BẮT BUỘC, không phải trang trí.
    ExperimentRunner gọi `model.fit(X, y.astype(int))` — tức là toàn bộ
    pipeline hiện tại CHỈ chạy được với nhãn phân loại. Đưa một nhãn hồi quy
    vào đó thì `forward_return = 0.03` bị `astype(int)` ép thành 0, mọi mẫu
    rơi hết về class 0, và mọi fold chết với "data contains only one class".
    Lỗi này biểu hiện thành `session.best = None` — trang UI dừng im lặng,
    người dùng không có cách nào biết vì sao.

    Có `_TASKS` thì UI lọc được dropdown và runner chặn được từ đầu.
    """
    def deco(fn):
        _LABELS[name] = fn
        _TASKS[name] = task
...
def available_labels(task: str | None = None) -> List[str]:
    """task=None -> tất cả; task="classification" -> chỉ nhãn dùng được hiện nay."""
    if task is None:
        return list(_LABELS.keys())
    return [n for n, t in _TASKS.items() if t == task]


def label_task(name: str) -> str:
...
@register_label("forward_return", task="regression")
...
@register_label("forward_volatility", task="regression")
